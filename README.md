...
可点击的多页面产品界面概念稿见 [花漾相绘 UI POC](poc/index.html)。

花漾相绘是一个面向普通用户的 AI 风格图片小程序。用户通过微信快捷登录，选择模板并上传 1–6 张图片；服务端按张扣除积分、异步生成作品，并提供历史记录与相册保存。新用户默认赠送 20 积分，积分用完后可通过微信支付充值。
...
- UI POC：发现、风格详情、上传、生成中、结果、作品、积分和个人中心的完整移动端交互原型。
