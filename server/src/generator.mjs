...
import { resolveEditSize } from './image-size.mjs'
...
async function mockGenerate(job, assets) {
  ...
  for (let index = 0; index < assets.length; index += 1) {
    ...
    try {
      ...
      results.push({ id: `${job.id}-${index + 1}`, assetId: asset.id, storagePath, mime: asset.mime })
    } catch (error) {
      failures.push({ assetId: asset.id, error: clipError(error.message || error) })
    }
  }
...
async function generateOneImage(job, template, asset, index) {
  ...
  const editSize = await resolveEditSize(source, config.image)
  ...
  if (!response.ok) {
    console.error(`[image:compatible] job=${job.id} asset=${asset.id} HTTP ${response.status}`, payload.raw?.slice(0, 500))
    throw new Error(extractApiError(response.status, payload))
  }

  const body = payload.json
  if (!body) throw new Error('生图服务返回了非 JSON 响应')
  const item = pickResultItem(body)
  if (!item) throw new Error('生图服务响应格式不支持（需要 data[0].b64_json 或 data[0].url）')
  ...
  if (sourceHash === outputHash) {
    throw new Error('生图结果与原图完全相同：请确认 IMAGE_PROVIDER=compatible、模型与提示词已生效')
  }

  const relativePath = path.join('outputs', job.userId, `${job.id}-${index + 1}-${asset.id.slice(0, 8)}${extensionForMime(mime)}`)
  const storagePath = relativePath.replaceAll('\\', '/')
  await mkdir(path.join(config.mediaDir, 'outputs', job.userId), { recursive: true })
  ...
  console.log(
    `[image:compatible] job=${job.id} image ${index + 1} ok in ${Date.now() - startedAt}ms ` +
    `size=${editSize || 'omitted'} bytes=${output.length}`
  )
  return { id: `${job.id}-${asset.id}`, assetId: asset.id, storagePath, mime }
}

async function compatibleGenerate(job, template, assets) {
  if (!config.image.apiKey) throw new Error('IMAGE_API_KEY 未配置，无法调用生图服务')
  if (!config.image.apiBase) throw new Error('IMAGE_API_BASE 未配置')
  if (!template?.prompt) throw new Error('模板未配置生图提示词，请在管理后台填写')

  console.log(
    `[image:compatible] job=${job.id} template=${template.id} model=${config.image.model} ` +
    `endpoint=${config.image.apiBase} images=${assets.length} sizeMode=${config.image.size || 'omit'} ` +
  ...
  for (let index = 0; index < assets.length; index += 1) {
    const asset = assets[index]
    try {
      results.push(await generateOneImage(job, template, asset, index))
    } catch (error) {
      const message = clipError(error.message || error)
      console.error(`[image:compatible] job=${job.id} asset=${asset.id} failed:`, message)
      failures.push({ assetId: asset.id, error: message })
    }
  }
  return { results, failures }
}
...
/**
 * Generate images one-by-one. Never throws for per-image failures.
 * Returns { results, failures }. Throws only for hard config errors before looping.
 */
